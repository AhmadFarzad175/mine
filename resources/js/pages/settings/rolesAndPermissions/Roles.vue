       ...
       <div class="btn-search pt-2">
         <div class="text-field">
           <v-text-field
             ...
             single-line
             hide-details
             v-model="SettingRepository.rolesearch"
           ></v-text-field>
         </div>
         <div class="btn">
             ...
             <v-row>
               <v-col>
                 <v-data-table-server
                   ...
                   v-model:items-per-page="SettingRepository.itemsPerPage"
                   :headers="headers"
                   :items-length="SettingRepository.totalItems"
                   :items="SettingRepository.roles"
                   :loading="SettingRepository.loading"
                   :search="SettingRepository.search"
                   @update:options="SettingRepository.fetchRolePermissions"
                   :item-key="SettingRepository.roles"
                   hover
...
//  const indeterminate = computed(() => selectedItems.value.length > 0 && selectedItems.value.length < SettingRepository.roles.length);
 
 const indeterminate = computed(() => {
  return selectedItems.value.length > 0 && selectedItems.value.length < SettingRepository.roles.length;
});

// Watch the selectedItems to automatically check/uncheck selectAll
watch(selectedItems, (newVal) => {
  selectAll.value = newVal.length === SettingRepository.roles.length;
});
 ...
 const toggleSelectAll = () => {
   if (selectAll.value) {
       ...
       selectedItems.value = SettingRepository.roles.slice();
   }
 };
 ...
 const headers = [
  //  { title: "", key: "checkbox", align: "start", sortable: false }, // Checkbox column
   ...
   { title: "Detail", key: "description", align: "left", sortable: false },
