<template>
        ...
        <v-row>
          ...
          <v-card subtitle="Expenses" hover>
            <v-card-text class="borderBT mb-4 mx-3">
              {{ ReportRepository.supplierReportDetails?.total_expenses || 'N/A' }} 
            </v-card-text>
          </v-card>
        </v-col>
        ...
        <v-col cols="12" md="3">
          <v-card subtitle="Total Expenses Amount" hover>
            <v-card-text class="borderBT mb-4 mx-3">
              {{ ReportRepository.supplierReportDetails?.total_amount || 'N/A' }} 
            </v-card-text>
          </v-card>
        </v-col>
        ...
        <v-col cols="12" md="3">
          <v-card subtitle="Total Expenses Payment" hover>
            <v-card-text class="borderNavy mb-4 mx-3">
              {{ ReportRepository.supplierReportDetails?.total_paid || 'N/A' }}
            </v-card-text>
          </v-card>
        </v-col>

        <v-col cols="12" md="3">
          <v-card subtitle="Total Expenses Due" hover>
            <v-card-text class="borderNavy mb-4 mx-3">
              {{ ReportRepository.supplierReportDetails?.total_due || 'N/A' }}
            </v-card-text>
          </v-card>
        </v-col>
    ...
    <v-tabs v-model="tab" align-tabs="end" color="deep-purple-accent-4">
      <v-tab :value="1">Bill Expenses</v-tab>
      <v-tab :value="2">Bill Expenses Payments</v-tab>
    </v-tabs>
    ...
    :items="ReportRepository.supplierReportDetails.bill_expenses"
    ...
    >
        ...
        <v-tabs-window-item :value="2">
          ...
          <v-data-table-server
            ...
            :items-length="ReportRepository.totalItems"
            :items="ReportRepository.supplierReportDetails.expense_payments"
            :loading="ReportRepository.loading"
            :item-key="'id'"
            hover
            class="mx-auto mt-4 no-padding-table"
  ...
  onMounted(async () => {
    await ReportRepository.supplierDetailsReports(routeParams.params.id);
    // loading.value = false;
  });
  ...
  const sentHeaders = [
   ...
   { title: "Paid", key: "paid", align: "left", sortable: false },
   { title: "Due", key: "due", align: "left", sortable: false },
 ];
...
const receivedHeaders = [
{
   ...
   { title: "Expense Ref", key: "expense_ref", align: "left", sortable: false },
   { title: "Amount", key: "amount", align: "left", sortable: false },
