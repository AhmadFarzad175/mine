<template>
    <StakeholderModal v-if="PeopleRepository.createDialog || PeopleRepository.updateDialog" />
...
//  const indeterminate = computed(() => selectedItems.value.length > 0 && selectedItems.value.length < PeopleRepository.stakeholders.length);
 ...
 const indeterminate = computed(() => {
  return selectedItems.value.length > 0 && selectedItems.value.length < PeopleRepository.stakeholders.length;
});
