                ...
                <v-card>
                    <v-card-title class="pt-4 d-flex justify-space-between">
                        <p class="font-weight-bold">&nbsp;
                            {{ PeopleRepository.updateDialog ? 'Edit Supplier' : 'Create Supplier' }}&nbsp;
                        </p>
...
onMounted(() => {
    if(PeopleRepository.updateDialog){
        formData.id =PeopleRepository.supplier?.id || '';
        formData.name =PeopleRepository.supplier?.name || '';
        formData.phone = PeopleRepository.supplier?.phone || '';
        formData.address = PeopleRepository.supplier?.address || '';
    }
    else { 
...
const submitForm = async () => {
    ...
    if (isValid) {
        if (PeopleRepository.updateDialog) {
            ...
            PeopleRepository.UpdateSupplier(formData.id,formData);
        } else {
