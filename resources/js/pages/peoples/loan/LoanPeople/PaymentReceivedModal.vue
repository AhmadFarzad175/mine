                ...
                <v-card class="px-3">
                    <v-card-title class="pt-4 d-flex justify-space-between">
                        <p class="font-weight-bold">
                            {{ PeopleRepository.updatePaymentReceivedDialog ? 'Edit Payment' : 'Create Payment' }}
                        </p>
...
const isDialogActive = computed({
    get() {
        return PeopleRepository.createPaymentReceivedDialog || PeopleRepository.updatePaymentReceivedDialog;
    },
    set(value) {
        PeopleRepository.createPaymentReceivedDialog = value;
        PeopleRepository.updatePaymentReceivedDialog = value;
    },
});
...
const closeDialog = () => {
    PeopleRepository.createPaymentReceivedDialog = false;
    PeopleRepository.updatePaymentReceivedDialog = false;
};
...
onMounted(() => {
    if (PeopleRepository.updatePaymentReceivedDialog) {
        // If we are editing an existing expense, fill the form with the existing data
        formData.id = PeopleRepository.receivedLoan?.id || '';
        formData.payment_date = PeopleRepository.receivedLoan?.payment_date || '';
        formData.amount = PeopleRepository.receivedLoan?.amount || '';
        formData.notes = PeopleRepository.receivedLoan?.notes || '';
        formData.currency = PeopleRepository.receivedLoan?.currency || {}; // Default to the first currency
        formData.loan_people_id= PeopleRepository.receivedLoan.loan_people_id;
...
const submitForm = async () => {
    ...
    if (isValid) {
        if (PeopleRepository.updatePaymentReceivedDialog) {
            PeopleRepository.updatePaymentReceived(formData.id, formData);
        } else {
            PeopleRepository.createPaymentReceived(formData);
        }
        closeDialog();
    }
};
