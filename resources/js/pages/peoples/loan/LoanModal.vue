                ...
                <v-card>
                    <v-card-title class="pt-4 d-flex justify-space-between">
                        <p class="font-weight-bold">&nbsp;
                            {{ PeopleRepository.updateDialog ? 'Edit Loan' : 'Create Loan' }}&nbsp;
                        </p>
...
onMounted(() => {
    if(PeopleRepository.updateDialog){
        ...
        formData.email = PeopleRepository.owner?.email || '';
    }
    else { 
...
const submitForm = async () => {
    ...
    if (isValid) {
        if (PeopleRepository.updateDialog) {
            ...
            PeopleRepository.updateLoan(formData.id,formData);
        } else {
            // Call the create method
            PeopleRepository.createLoan(formData);
        }
        closeDialog();
    }
};
