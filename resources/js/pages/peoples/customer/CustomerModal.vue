                ...
                <v-card>
                    <v-card-title class="pt-4 d-flex justify-space-between">
                        <p class="font-weight-bold">&nbsp;
                            {{ PeopleRepository.updateDialog ? 'Edit Customer' : 'Create Customer' }}&nbsp;
                        </p>
...
onMounted(() => {
    if(PeopleRepository.updateDialog){
        formData.id =PeopleRepository.customer?.id || '';
        formData.name =PeopleRepository.customer?.name || '';
        formData.phone = PeopleRepository.customer?.phone || '';
        formData.address = PeopleRepository.customer?.address || '';
    }
    else { 
