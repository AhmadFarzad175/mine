                ...
                <v-card class="px-3">
                    <v-card-title class="pt-4 d-flex justify-space-between">
                        <p class="font-weight-bold">
                            {{ ExpensesRepository.updatePaymentDialog ? 'Edit Payment' : 'Create Payment' }}
                        </p>
...
const formData = reactive({
    ...
    description: "",
    currencyIndex: 0,
    currency:ExpensesRepository.expense?.currency, // Default to the first currency
    bill_expenses_id: ExpensesRepository.bill_expenses_id,
});
...
const isDialogActive = computed({
    get() {
        return ExpensesRepository.createPaymentDialog || ExpensesRepository.updatePaymentDialog;
    },
    set(value) {
        ExpensesRepository.createPaymentDialog = value;
        ExpensesRepository.updatePaymentDialog = value;
    },
});
...
const closeDialog = () => {
    ExpensesRepository.createPaymentDialog = false;
    ExpensesRepository.updatePaymentDialog = false;
};
...
onMounted(() => {
    if (ExpensesRepository.updatePaymentDialog) {
        ...
        formData.amount = ExpensesRepository.expense?.amount || '';
        formData.description = ExpensesRepository.expense?.description || '';
        formData.currency = ExpensesRepository.expense?.currency || {}; // Default to the first currency
        formData.bill_expenses_id = ExpensesRepository.bill_expenses_id;
...
const submitForm = async () => {
    ...
    if (isValid) {
        if (ExpensesRepository.updatePaymentDialog) {
            ExpensesRepository.UpdateBillExpensePayment(formData.id, formData);
        } else {
            ExpensesRepository.createPayment(formData);
        }
        closeDialog();
    }
};
