<template>
    ...
    <div>
        <v-dialog
            ...
            width="50rem"
            v-model="ExpensesRepository.viewPaymentDialog"
        >
                    ...
                    <div class="d-flex flex-column body-2 px-2 py-4">
                        <v-table class="payment-table">
                            ...
                            <tbody>
                                <tr
                                    class="text-left"
                                    v-for="(item, index) in ExpensesRepository.billExpensePayments"
                                    :key="index"
                                >
                                    <td>{{ item.date }}</td>
                                    <td>{{ item.amount }}</td>
...
const deleteItem = async (item) => {
    await ExpensesRepository.DeleteBillExpensePayment(item.id);
};

const editItem = async (item) => {
    console.log(item)
    // ExpensesRepository.productId = item.id;
    ExpensesRepository.expense = item
    // ExpensesRepository.FetchBillExpensePayment(item.id).then(() => {
        ExpensesRepository.updatePaymentDialog = true;
    // });
};
</script>
