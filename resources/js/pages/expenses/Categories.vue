<template>
    <ProductModalCategory v-if="ExpensesRepository.createDialog || ExpensesRepository.updateDialog" />
       ...
       <div class="btn-search pt-2">
         <div class="text-field">
           <v-text-field
             ...
             single-line
             hide-details
             v-model="ExpensesRepository.categoriesearch"
           ></v-text-field>
         </div>
         <div class="btn">
             ...
             <v-row>
               <v-col>
                 <v-data-table-server
                   ...
                   :search="ExpensesRepository.categoriesearch"
                   @update:options="ExpensesRepository.fetchExpensesCategory"
                   :item-key="ExpensesRepository.categories"
                   hover
...
//  const indeterminate = computed(() => selectedItems.value.length > 0 && selectedItems.value.length < ExpensesRepository.categories.length);
 
 const indeterminate = computed(() => {
  return selectedItems.value.length > 0 && selectedItems.value.length < ExpensesRepository.categories.length;
});

// Watch the selectedItems to automatically check/uncheck selectAll
watch(selectedItems, (newVal) => {
  selectAll.value = newVal.length === ExpensesRepository.categories.length;
});
 ...
 const toggleSelectAll = () => {
   if (selectAll.value) {
       ...
       selectedItems.value = ExpensesRepository.categories.slice();
   }
 };
 ...
 const headers = [
   ...
   { title: "Detail", key: "details", align: "left", sortable: false },
...
//  };
 ...
 const deleteItem = async (item) => {
   await ExpensesRepository.DeleteExpenseCategory(item.id);
};

const editItem = (category) => {
   ExpensesRepository.category = category;
