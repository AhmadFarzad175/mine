<template>
   <PaymentModal v-if="ExpensesRepository.createPaymentDialog || ExpensesRepository.updatePaymentDialog" />
   <ViewPayment v-if="ExpensesRepository.viewPaymentDialog" />
        ...
        <div class="btn-search pt-2">
          <div class="text-field">
            <v-text-field
              ...
              single-line
              hide-details
              v-model="ExpensesRepository.billableExpenseSearch"
            ></v-text-field>
          </div>
          <div class="btn">
            ...
            &nbsp;
            <router-link to="/createBillableExpense">
              ...
              <v-row>
                <v-col>
                  <v-data-table-server
                    ...
                    :items="ExpensesRepository.billableExpenses"
                    :loading="ExpensesRepository.loading"
                    :search="ExpensesRepository.billableExpenseSearch"
                    @update:options="ExpensesRepository.FetchBillableExpense"
                    :item-key="ExpensesRepository.billableExpenses"
                    hover
  ...
  const indeterminate = computed(() => selectedItems.value.length > 0 && selectedItems.value.length < ExpensesRepository.billableExpenses.length);
  ...
  const toggleSelectAll = () => {
    if (selectAll.value) {
        ...
        selectedItems.value = ExpensesRepository.billableExpenses.slice();
    }
  };
  ...
  const headers = [
    ...
    },
    {
      ...
      align: "center",
      sortable: false,
      key: "Due_with_currency",
      color: "red",
    },
    { title: "Grand Total", key: "amount_with_currency", align: "center", sortable: false },
  ...
  const deleteItem = async (item) => {
    await ExpensesRepository.DeleteBillableExpense(item.id);
};
  ...
  const CreatePaymentDialog = (item) => {
    ...
    ExpensesRepository.bill_expenses_id= item.id;
    
    ExpensesRepository.expense=item;

    ExpensesRepository.createPaymentDialog = true;
  };

  const ViewPaymentDialog = (item) => {
      // console.log(item.id);
      // const earningId = item.id;
      // ExpensesRepository.updatePaymentId = item.id;
  ExpensesRepository.bill_expenses_id = item.id; 
      ExpensesRepository.billExpensePayments =[];
      if (ExpensesRepository.billExpensePayments.length === 0) {
          ExpensesRepository.FetchBillExpensePayments(item.id)
              .then(() => {
                  ExpensesRepository.viewPaymentDialog = true;
              })
              .catch((error) => {
